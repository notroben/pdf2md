...
def run_conversion_job(params: dict) -> None:
    """Execute a single PDF to Markdown conversion job and render UI."""
...
Examples:
  ...
  pdf2md --interactive
    ...
    if is_interactive:
        # Interactive loop: allows converting multiple documents in a single session
        while True:
            try:
                params = ui.interactive_wizard()
                run_conversion_job(params)

                # Post-conversion menu: convert another or exit
                if not ui.prompt_post_conversion():
                    break
        ...
        # Command-line mode: runs once and returns to shell
        try:
